**Italiano** · versione inglese del kit: [English](https://github.com/uniurbit/memento-ai-docs/blob/main/README.md)
...
**Licenza:** CC BY 4.0
**Lingua.** La versione inglese è il testo ufficiale, il presente documento rappresenta la traduzione in lingua italiana. Si lavora in una lingua sola: vedi [Lingua del kit](#63-lingua-del-kit).
...
- [6. Fase 0 – Ambiente](#6-fase-0--ambiente)
  ...
  - [6.3 Lingua del kit](#63-lingua-del-kit)
...
- [Appendice tecnica 5 – Le lingue del kit](#appendice-tecnica-5--le-lingue-del-kit)
...
```
progetto/
├── PROJECT_BRIEF.md   mandato del progetto; si legge per primo
├── README.md           il presente documento
├── CONTEXT/           documentazione di riferimento acquisita dall'ente
├── images/           figure del presente documento
├── RELEASE/           versioni finali esportate
├── TOOLS/          script di cattura delle interazioni (App. tecnica 2)
└── REGISTERS/
...
Il kit è distribuito in più di una lingua: l'inglese sta nella radice del repository, le altre lingue in `lang/`, ciascuna in una cartella propria che riproduce la stessa struttura con gli stessi nomi di file.
L'albero qui sopra è quindi anche la struttura di lavoro, cioè ciò che resta dopo la scelta della lingua prescritta in [Lingua del kit](#63-lingua-del-kit).
...
Si trova nella cartella `CONTEXT/`.
...
**Documento di avvio.** Il file `PROJECT_BRIEF.md`, nella cartella principale del progetto, che contiene il mandato: è la prima lettura di ogni sessione.
...
![Le sei fasi del metodo e i passaggi che le collegano, descritti nella tabella e nel testo che seguono.](images/method-architecture.png)
...
L'inglese sta nella radice del repository; ogni altra lingua sta in `lang/<codice>/`, in una cartella che riproduce la struttura della radice con gli stessi nomi di file.

**Una sola lingua DEVE essere scelta prima di cominciare, e `lang/` DEVE essere rimossa.**
Tenere due copie di documenti vincolanti significa vederle divergere col tempo: una viene aggiornata e l'altra no, nessuno se ne accorge, e chi legge quella sbagliata lavora su regole superate.
Rimuovere la cartella trasforma un rischio permanente in una decisione presa una volta sola, ed è ciò che rende i nomi dei file del metodo identici in ogni lingua.

La scelta si esegue all'inizializzazione del repository ([Inizializzazione del repository](#721-inizializzazione-del-repository)), con un comando solo per ciascuna lingua, e il suo esito viene controllato all'apertura di ogni sessione dalla regola di `REGISTERS/INSTRUCTIONS.md`: se `lang/` è ancora presente, il modello lo segnala e chiede prima di procedere.
La procedura per ciascuna lingua, ciò che deve restare identico fra le lingue e il modo di aggiungerne una nuova sono nell'[Appendice tecnica 5](#appendice-tecnica-5--le-lingue-del-kit).
...
2. **Condizioni contrattuali del servizio** usato, con particolare attenzione all'eventuale riutilizzo dei contenuti trasmessi per addestrare i modelli.
   ...
   La verifica DEVE essere fatta sul contratto o sul pannello di amministrazione e DEVE essere annotata, con la data, in `REGISTERS/PROJECT.md`.
...
- **`CONTEXT/`** – i documenti di riferimento.
...
- **`RELEASE/`** – le versioni finali esportate.
...
- **`REGISTERS/`** – i quattro registri del progetto, descritti qui sotto.
...
cp -r lang/it/. .  # porta l'italiano nella radice
rm -rf lang        # e rimuove le altre lingue
...
I due comandi centrali eseguono la scelta della lingua prescritta in [Lingua del kit](#63-lingua-del-kit): sovrascrivono i documenti inglesi della radice con quelli italiani e rimuovono la cartella delle lingue.
Per lavorare in inglese basta il solo `rm -rf lang`.
In un caso e nell'altro l'operazione è un atto unico e indivisibile: non si può eseguire a metà, ed è la ragione per cui è scritta così (Appendice tecnica 5).
...
Il mandato del progetto sta in `PROJECT_BRIEF.md`, nella cartella principale.
...
3. **Rinvio vincolante alle regole operative.** Il documento di avvio impone di leggere per intero `REGISTERS/INSTRUCTIONS.md` e di attenersi a quel file per tutta la lavorazione.
...
- In `CONTEXT/` DEVE essere raccolta tutta la documentazione pertinente: normativa sovraordinata, atti collegati, versioni precedenti, verbali, requisiti.
...
Ho letto PROJECT_BRIEF.md e, come prescritto, REGISTERS/INSTRUCTIONS.md,
REGISTERS/PROJECT.md e REGISTERS/WORKLOG.md; ho poi verificato CONTEXT/.
...
delle attività sono vuoti: è la prima sessione del progetto. In CONTEXT/
...
```
Leggi tutti i file in CONTEXT/ e produci il manifest delle fonti:
...
Ho letto nove file e scritto CONTEXT/MANIFEST.md.
...
Vanno verificati, e se i documenti esistono vanno aggiunti a `CONTEXT/` e censiti nel manifest.
...
CONTEXT/. Dove una decisione di merito non risulta dal mandato o dalle
...
```
Aggiungi a CONTEXT/ il provvedimento del Garante che avevi segnalato come
...
La prescrizione è in `REGISTERS/INSTRUCTIONS.md`.
...
La procedura operativa completa (riconoscimento, ordine di esecuzione, rimozione, trattamento delle annotazioni non eseguite o non conformi, archiviazione preventiva) è in `REGISTERS/INSTRUCTIONS.md`, il file che il modello legge a ogni sessione.
...
```
Verifica il documento rispetto a tutte le fonti in CONTEXT/: riferimenti
...
- L'art. 5 prevede il consenso del dipendente all'accesso alla casella; il
  provvedimento del Garante richiamato in CONTEXT/ fonda invece
...
- **Esportazione.** La versione approvata è convertita dal formato di lavoro ai formati standard dell'ente e depositata in `RELEASE/`.
...
| Subentra personale nuovo, che altrimenti dovrebbe intervistare chi ha lavorato prima | `PROJECT_BRIEF.md` e `PROJECT.md`, che rendono l'intervista superflua |
...
Nel metodo i comandi non si eseguono a mano: li esegue il modello, al termine di ciascun giro del ciclo di produzione e a ogni passaggio di fase, secondo le istruzioni contenute in `REGISTERS/INSTRUCTIONS.md`.
Alla persona restano la decisione nel merito, l'annotazione delle bozze e la verifica di ciò che è stato registrato.
Come si inizializza il repository e quale convenzione seguono i messaggi di commit è descritto nella Fase 1; i divieti che non ammettono eccezioni sono in `REGISTERS/INSTRUCTIONS.md`; il lavoro di più redattori sullo stesso corpus è nell'[Appendice tecnica 4](#appendice-tecnica-4--collaborazione-fra-più-redattori).
...
Le modalità di scrittura di ciascun registro (modifica sul posto, aggiunta in fondo, alimentazione automatica) sono descritte in `REGISTERS/INSTRUCTIONS.md`.
...
Questo repository fornisce l'implementazione (`TOOLS/record_interaction.py`, Python 3 e sola libreria standard) con l'avvertenza che l'aggancio è l'unico punto in cui il metodo tocca un prodotto specifico: adattarlo a un altro strumento richiede di modificare una sola parte dello script.
...
```
evento «richiesta inviata»    → comando: python3 TOOLS/record_interaction.py
                                tempo massimo: 10 secondi

evento «risposta completata»  → comando: python3 TOOLS/record_interaction.py
...
Le prescrizioni operative dei due casi sono in `REGISTERS/INSTRUCTIONS.md`.
...
## Appendice tecnica 5 – Le lingue del kit

### Come sono disposte le lingue

L'inglese sta nella radice del repository. Ogni altra lingua sta in `lang/<codice>/`, dove `<codice>` è il codice a due lettere della lingua (`it` per l'italiano), in una cartella che riproduce la radice **con gli stessi nomi di file**:
...
Questo repository non è un insieme di documenti da leggere: è lo scheletro di lavoro di un progetto, e i suoi file devono stare a percorsi prescritti: `REGISTERS/INSTRUCTIONS.md` si legge a ogni sessione, lo script di cattura scrive in `REGISTERS/CONVERSATION.md`, il file di avvio dello strumento rinvia a `PROJECT_BRIEF.md`.

### Che cosa contiene la cartella di una lingua, e che cosa non contiene

Una cartella sotto `lang/` contiene soltanto ciò che cambia con la lingua:
questa specifica, il documento di avvio, i quattro registri e, dove è tradotta,
la figura. Non è una copia autonoma del kit.

Tutto ciò che non cambia con la lingua vive una volta sola, nella radice:

| Solo nella radice | Perché |
| :---- | :---- |
| `TOOLS/record_interaction.py` | È l'unico file di codice del kit. Due copie significherebbero una correzione applicata a una e non all'altra: esattamente il difetto che questo assetto serve a evitare. Docstring e commenti sono in inglese in ogni lingua del kit. |
| `LICENSE`, `LICENSES/`, `REUSE.toml` | Una licenza sola, dichiarata una volta, file per file. Duplicare la dichiarazione indebolirebbe proprio ciò per cui esiste. |
| `.gitignore` | Sono pattern, non prosa. |
| `CONTEXT/`, `RELEASE/` | Cartelle di lavoro, vuote per progetto: le riempie l'ente adottante. |

La scelta della lingua è quindi una **sovrapposizione**, non uno spostamento: i
documenti di una lingua si sovrappongono alla radice, che è già completa, e la
cartella delle lingue viene rimossa. Ciò che resta è un progetto completo in una
lingua sola.

### Avviare un progetto in una lingua

La scelta si fa una volta sola, all'inizializzazione del repository ([Inizializzazione del repository](#721-inizializzazione-del-repository)), e consiste nel portare una lingua nella radice e rimuovere le altre:

| Lingua di lavoro | Comando |
| :---- | :---- |
| Italiano | `cp -r lang/it/. .` e poi `rm -rf lang` |
| Inglese | `rm -rf lang` |
| altra lingua | `cp -r lang/<codice>/. .` e poi `rm -rf lang` |

**È un atto unico e indivisibile, e questo è il punto.** Un'operazione da eseguire file per file finisce per essere eseguita a metà: un progetto si ritrova con la specifica in una lingua e le regole operative nell'altra, e nessuno se ne accorge finché non sono le regole a contare. Portare nella radice una cartella intera o avviene o non avviene.

Lo stesso ragionamento vale per la tentazione opposta. Tenere entrambe le lingue dentro il progetto, riservandosi di cancellarne una più avanti, produce due documenti vincolanti che dicono le stesse regole: uno viene aggiornato, l'altro no, e chi legge quello sbagliato lavora su regole superate. Chi vuole una seconda lingua per leggerla la tiene **fuori** dal progetto.

Fatta la scelta, `lang/` non esiste più nel progetto adottante. La sua presenza è un difetto, e la regola di `REGISTERS/INSTRUCTIONS.md` fa sì che il modello la segnali all'apertura di ogni sessione.

### Che cosa resta identico fra le lingue

- **I nomi di file e cartelle**, come sopra. Sono struttura, non contenuto.
- **La numerazione di sezioni e appendici.** I rimandi citano numeri, e li citano anche i registri di un progetto: la sezione 8.4 deve essere l'analisi di completezza in ogni lingua.
- **I delimitatori delle annotazioni** `<<<` e `>>>`, descritti in [Revisione umana](#83-azione-23--revisione-umana): li riconosce il modello, non li legge una persona.
- **Marcatori ed etichette dell'archivio delle interazioni** (`<!-- interaction:… -->`, `## Request`, `capture: environment`), che sono in inglese in ogni lingua del kit, perché lo script di cattura è uno solo. Il testo registrato resta verbatim nella lingua in cui è stato scritto.
- **Le intestazioni SPDX** e la dichiarazione di licenza leggibile da una macchina (`REUSE.toml`).
- **I tipi dei messaggi di commit** (`feat`, `fix`, `docs`, `refactor`, `chore`, `meta`, `checkpoint`) e la convenzione `tipo(ambito): descrizione`.

Non si traducono: `LICENSE` e il contenuto di `LICENSES/`, il cui testo che fa fede è quello pubblicato da Creative Commons; e i nomi con cui uno strumento denomina i propri eventi, che appartengono al prodotto.

### Aggiungere una lingua

1. Copiare i documenti della radice in `lang/<codice>/`, conservando la struttura: `README.md`, `PROJECT_BRIEF.md`, i quattro file di `REGISTERS/` e, se la figura viene tradotta, `images/method-architecture.png`.
2. Tradurre la prosa. Nomi, numerazione ed elementi elencati sopra restano invariati.
3. Rendere le parole chiave di [Parole chiave](#2-parole-chiave) con gli equivalenti consolidati della lingua d'arrivo. Dove una lingua non abbia una resa univoca di DEVE / NON DEVE / DOVREBBE / PUÒ, **conservare le parole chiave inglesi in maiuscolo** invece di scegliere un'approssimazione: tutto il documento si regge sulla loro univocità.
4. Tradurre la figura, oppure lasciare quella inglese: un diagramma non tradotto è un limite, un diagramma tradotto male è un difetto.
5. Non tradurre i marcatori del record, i delimitatori e i tipi di commit.
6. Proporre la lingua con una pull request. Una traduzione che resta a chi l'ha fatta non serve a nessuno.
